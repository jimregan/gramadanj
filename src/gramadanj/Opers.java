...
public class Opers {
	...
	public static String Demutate (String text) {
		...
		ret=s(ret, "^[dD]'([aeioua‡Ž’—œAEIOUçƒêîò].*)$", "$1");
		ret=s(ret, "^[hH]([aeioua‡Ž’—œAEIOUçƒêîò].*)$", "$1");
		ret=s(ret, "^[nN]-([aeioua‡Ž’—œAEIOUçƒêîò].*)$", "$1");
	...
	public static String Mutate (Mutation mutation, String text) {
		...
		if ((mutation == Mutation.Len1) || (mutation == Mutation.Len1D)
			...
			if ((mutation == Mutation.Len1) || (mutation == Mutation.Len1D)) {
				...
				ret=s(ret, "^([sS])([rnlRNLaeiou‡Ž’—œAEIOUçƒêîò].*)$", "$1h$2");
			} else {
				ret=s(ret, "^([pbmfcgPBMFCG])(.*)$", "$1h$2");
				if ((mutation == Mutation.Len3) || (mutation == Mutation.Len3D)) {
					ret = s(ret, "^([sS])([rnlRNLaeiou‡Ž’—œAEIOUçƒêîò].*)$", "t$1$2");
				}
			}
			if ((mutation == Mutation.Len1D) || (mutation == Mutation.Len2D) 
				|| (mutation == Mutation.Len3D)) {
				// FIXME: f???
				ret = s(ret, "^([aeiou‡Ž’—œAEIOUçƒêîòfF])(.*)$", "d'$1$2");
				//ret = s(ret, "^([aeiou‡Ž’—œAEIOUçƒêîò])(.*)$", "d'$1$2");
				//ret = s(ret, "^([fF])(.*)$", "d'fh$2");
			}
		}
		...
		if (mutation == Mutation.PrefT) {
			ret=s(ret, "^([aeiuo‡Ž’œ—])(.*)$", "t-$1$2");
			ret=s(ret, "^([AEIUOçƒêòî])(.*)$", "t$1$2");
		}
		if (mutation == Mutation.PrefH) {
			ret=s(ret, "^([aeiuo‡Ž’œ—AEIUOçƒêòî])(.*)$", "h$1$2");
		}
		return ret;
	}
	
	public static String Consonants = "bcdfghjklmnpqrstvwxz";
	public static String Vowels = "aeiou‡Ž’—œ";
	public static String VowelsBroad = "aou‡—œ";
	public static String VowelsSlender = "eiŽ’";
	
	public static String Broaden(String bayse) {
		String ret = bayse;
		String[] sources = new String[] {"—i", "ei", "Ži", "i",  "a’",  "’",  "ui", "io"};
		String[] targets = new String[] {"—",  "ea", "Ža", "ea", "a’o", "’o", "o",  "ea"};
		Matcher m;
	...
	public static boolean EndsVowel(String bayse) {
		return bayse.matches("[aeiou‡Ž’—œAEIOUçƒêîò]$");
	}
